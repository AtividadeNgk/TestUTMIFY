...
# ===== COMANDO TEMPORÁRIO DE TESTE - REMOVER DEPOIS =====
async def test_tracking(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """Comando para testar o rastreamento UTM - TEMPORÁRIO"""
    ...
    message = "🧪 <b>TESTE DE RASTREAMENTO</b>\n\n"
    ...
    if tracking_data:
        ...
        message += "Exemplo:\n"
        message += f"<code>t.me/{context.bot.username}?start=fbclid-ABC123_utm-source-facebook</code>\n"
    ...
    try:
        ...
        if config_data.get('utmify_token'):
            from modules.tracking import UTMifyTracker
            tracker = UTMifyTracker(config_data['utmify_token'])
            
            # Envia um pedido de teste
            test_order = {
                ...
                "isTest": True  # IMPORTANTE: Define como teste
            }
            
            result = tracker.send_order(test_order)
            
            if result.get('success'):
                message += "✅ Conexão OK! Evento de teste enviado.\n"
            else:
...
async def run_bot(token, bot_id):
    ...
    bot_application.add_handler(CommandHandler('test_tracking', test_tracking))  # TEMPORÁRIO - REMOVER DEPOIS
