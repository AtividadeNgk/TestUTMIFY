...
def criar_pix_pp(token, valor_cents, user_id=None, bot_id=None):
    # Endpoint da API
    url = "https://api.pushinpay.com.br/api/pix/cashIn"

    valor_cents = float(valor_cents)
    
    # ===== NOVO: Busca dados de tracking =====
    metadata = {}
    ...
    data = {
        ...
        "external_id": user_id if user_id else None,  # ← NOVO
        "metadata": metadata if metadata else None,    # ← NOVO
        "split_rules": [
            {
                "value": comissao,
                "account_id": "9D60FF2D-4298-4AEF-89AB-F27AE6A9D68D"
